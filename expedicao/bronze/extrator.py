"""
Motor genérico de extração da camada Bronze da Expedição.

Mesmo padrão do Comercial/Produção/Estoque/Laudos RMA: um único motor
genérico serve para as 2 tabelas do catálogo (expedicao/bronze/tabelas.py)
-- não precisa de script por tabela.

Para cada tabela:
    1. Verifica se ela já existe e tem dados na Bronze (tabela_tem_dados).
       Exceção: USU_V120EST (chaves_pk=None) pula essa checagem -- é
       view sem PK, sempre full_reload, mesmo se já tiver dados (ver
       core/loader.py carregar_bronze()).
    2. Monta a query de extração:
        - 1ª carga         -> SELECT * com filtro pela coluna de empresa
                              ...
                              quando tem_codfil=True. Tabelas globais
                              (tem_codemp=False, ex.: R900GRP): sem
                              filtro algum.
        - cargas seguintes -> idem + filtro coluna_data >= SYSDATE - 60
                              (nenhuma das 2 tabelas desta área tem
                              coluna_data -- ambas full_reload sempre,
                              ver tabelas.py)
    3. Chama core.loader.carregar_bronze() para gravar na Bronze.

TABELAS COMPARTILHADAS COM O COMERCIAL (E120PED, E069GRE, E085CLI) NÃO
aparecem em TABELAS_DESTE_BLOCO -- já são mantidas pelo
comercial/bronze/extrator.py. Ver observação em bronze/tabelas.py.

VERIFICAÇÃO DE ÓRFÃOS: mesma regra do Comercial/OPEX/Laudos RMA/Produção/
Estoque -- só roda na ÚLTIMA execução do dia, quando o script é chamado
com a flag --sweep-orfaos.
"""
...
from expedicao.bronze.tabelas import TABELAS, buscar_tabela
from expedicao.config.settings import (
    CODEMP_AQUARIO,
    CODFIL_AQUARIO,
...
def montar_query(info: dict, primeira_carga: bool) -> str:
    ...
    Respeita tem_codemp: tabelas globais (tem_codemp=False, ex.:
    R900GRP) não recebem nenhum filtro de empresa. Para as demais,
    filtra pela coluna real de empresa (info["coluna_codemp"]) = 1, e
    pela coluna real de filial (info["coluna_codfil"]) = 1 quando
    tem_codfil = True.
    ...
    Tabelas sem PK (chaves_pk=None, ex.: USU_V120EST) sempre chamam esta
    função com primeira_carga=True -- não existe conceito de incremental
    pra elas (ver rodar_tabela()).
    """
    tabela = info["tabela"]
    filtros = []
...
def montar_query_pks(info: dict) -> str:
    ...
    Não é chamada para tabelas sem PK (ver rodar_tabela()).
    """
    tabela  = info["tabela"]
...
def montar_query_contagem(info: dict) -> str:
    """
    Monta a query de contagem completa (sem janela de data), usada pela
    conferência. Diferente de montar_query_pks, funciona também para
    tabelas sem PK física (chaves_pk=None, ex.: USU_V120EST) -- não
    depende de nenhuma coluna específica, só do escopo empresa/filial.
    """
    tabela = info["tabela"]
    filtros = []
    ...
    if filtros:
        where = " AND ".join(filtros)
        return f"SELECT COUNT(*) FROM SAPIENS.{tabela} WHERE {where}"
    else:
        return f"SELECT COUNT(*) FROM SAPIENS.{tabela}"
...
def rodar_tabela(engine, nome_tabela: str, verificar_orfaos: bool = False) -> dict:
    ...
    sem_pk = info["chaves_pk"] is None

    # Tabela sem PK (view, ex.: USU_V120EST): não existe conceito de
    # incremental -- sempre full, sempre sem janela de data.
    primeira_carga = True if sem_pk else not tabela_tem_dados(engine, schema_bronze, nome_tabela)
    query = montar_query(info, primeira_carga)

    print(f"\n{'='*60}")
    if sem_pk:
        print(f"  TABELA: {nome_tabela}  (VIEW SEM PK -- SEMPRE FULL_RELOAD)")
    else:
        print(f"  TABELA: {nome_tabela}  ({'1a CARGA - FULL' if primeira_carga else 'INCREMENTAL'})")
    print(f"{'='*60}")
    print(f"  Query: {query}")

    coluna_ordem = (info["coluna_data"] or info["chaves_pk"][0]) if not sem_pk else None
    query_pks_completo = montar_query_pks(info) if (verificar_orfaos and not sem_pk) else None
...
if __name__ == "__main__":
    ...
    if verificar_orfaos:
        ...
        print("  (tabelas sem PK, como USU_V120EST, não entram nessa verificação --")
        print("  já são sempre full_reload, o que já reflete o estado atual)")
    else:
