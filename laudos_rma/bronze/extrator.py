"""
Motor genérico de extração da camada Bronze do Laudos RMA.

Mesmo padrão do Comercial: um único motor genérico serve para as 9
tabelas do catálogo (laudos_rma/bronze/tabelas.py) -- não precisa de
script por tabela.

Para cada tabela:
    1. Verifica se ela já existe e tem dados na Bronze (tabela_tem_dados).
       Exceção: USU_VZRASLAU (chaves_pk=None) pula essa checagem -- é
...
TABELAS COMPARTILHADAS COM O COMERCIAL (E440NFC, E095FOR, E075PRO,
E075DER, E013AGP, E140NFV, E073TRA, E140IPV, E140IDE, E001TNS) NÃO
aparecem em TABELAS_DESTE_BLOCO -- já são mantidas pelo
comercial/bronze/extrator.py. Ver observação em bronze/tabelas.py.

VERIFICAÇÃO DE ÓRFÃOS: mesma regra do Comercial/OPEX -- só roda na
ÚLTIMA execução do dia, quando o script é chamado com a flag
--sweep-orfaos.
"""
...
from laudos_rma.bronze.tabelas import TABELAS, buscar_tabela
from laudos_rma.config.settings import (
    CODEMP_AQUARIO,
    CODFIL_AQUARIO,
...
def montar_query(info: dict, primeira_carga: bool) -> str:
    ...
    empresa (info["coluna_codemp"]) = 1, e pela coluna real de filial
    ...
    Tabelas sem PK (chaves_pk=None, ex.: USU_VZRASLAU) sempre chamam esta
...
def rodar_tabela(engine, nome_tabela: str, verificar_orfaos: bool = False) -> dict:
    ...
    # Tabela sem PK (view, ex.: USU_VZRASLAU): não existe conceito de
...
if __name__ == "__main__":
    ...
    if verificar_orfaos:
        ...
        print("  (tabelas sem PK, como USU_VZRASLAU, não entram nessa verificação --")
