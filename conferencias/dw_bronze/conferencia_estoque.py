"""
Conferência Bronze x Sapiens -- Estoque.
...
anterior tinha os mesmos 2 pontos cegos:
    ...
    a) Contagem usava o MESMO filtro (tem_codemp/tem_codfil) do catálogo
       pros dois lados -- então o bug de tem_codfil=True em E420IPO/
       E420OCP (corrigido em 17/07/2026, ver estoque/bronze/tabelas.py)
       nunca teria aparecido aqui.
    b) Só comparava COUNT(*) -- nunca pegaria divergência de valor num
       registro presente dos dois lados.
    c) montar_query_pks() usa ", ".join(info["chaves_pk"]) -- quebraria
       (TypeError) numa tabela sem PK física (chaves_pk=None). Nenhuma
       tabela desta área tem esse caso hoje, mas o guard foi adicionado
       mesmo assim, por consistência com as demais conferências.

Só leitura -- não corrige nem apaga nada.

Uso:
    python -m conferencias.dw_bronze.conferencia_estoque
        -> contagem (rápida) nas tabelas do catálogo.

    python -m conferencias.dw_bronze.conferencia_estoque --conteudo E420IPO
...
from estoque.bronze.extrator import TABELAS_DESTE_BLOCO, montar_query, montar_query_pks
from estoque.bronze.tabelas import buscar_tabela
from estoque.config.settings import CODEMP_AQUARIO, schema_bronze
...
if __name__ == "__main__":
    ...
    if "--conteudo" in sys.argv:
        ...
        try:
            ...
            print("  Uso: python -m conferencias.dw_bronze.conferencia_estoque --conteudo NOME_TABELA")
            sys.exit(1)
