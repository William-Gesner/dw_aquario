"""
Conferência Bronze x Sapiens -- Rastreabilidade.
...
anterior tinha os mesmos 2 pontos cegos:
    ...
    a) Contagem usava o MESMO filtro (tem_codemp/tem_codfil) do catálogo
       pros dois lados -- nesta área isso não escondeu nenhum bug real
       (USU_T140QRC está com tem_codfil correto, confirmado em 17/07/2026
       direto no legado vbirastreabilidade.py: a tabela-âncora E140IPV já
       fixa CODFIL=1 no WHERE, então o casamento dinâmico de USU_T140QRC
       com QRC.USU_CODFIL é transitivamente correto), mas o ponto cego em
       si existia -- mantido visível via ALERTA por consistência com as
       demais áreas.
    b) Só comparava COUNT(*) -- nunca pegaria divergência de valor num
       registro presente dos dois lados. Relevante aqui: catálogo alerta
       que USU_DATGER é NULLABLE e pode deixar linhas de fora do
       incremental permanentemente -- só a checagem de conteúdo pega isso.

Só leitura -- não corrige nem apaga nada.

Uso:
    python -m conferencias.dw_bronze.conferencia_rastreabilidade
        -> contagem (rápida) na tabela do catálogo.

    python -m conferencias.dw_bronze.conferencia_rastreabilidade --conteudo USU_T140QRC
        -> conteúdo (MINUS, dado a dado).
"""
...
from rastreabilidade.bronze.extrator import TABELAS_DESTE_BLOCO, montar_query, montar_query_pks
from rastreabilidade.bronze.tabelas import buscar_tabela
from rastreabilidade.config.settings import CODEMP_AQUARIO, schema_bronze
...
# ----- CONFERÊNCIA DE CONTEÚDO (dado a dado) -----
...
if __name__ == "__main__":
    ...
    if "--conteudo" in sys.argv:
        ...
        try:
            ...
            print("  Uso: python -m conferencias.dw_bronze.conferencia_rastreabilidade --conteudo NOME_TABELA")
            sys.exit(1)
