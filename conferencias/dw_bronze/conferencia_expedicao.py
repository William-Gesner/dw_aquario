"""
Conferência Bronze x Sapiens -- Expedição.
...
anterior tinha os mesmos 2 pontos cegos:
    ...
    a) Contagem usava o MESMO filtro (tem_codemp/tem_codfil) do catálogo
       pros dois lados -- nesta área isso não escondeu nenhum bug (as 2
       tabelas -- R900GRP e USU_V120EST -- estão com tem_codfil correto,
       confirmado em 17/07/2026 direto no legado vbiexpedicao.py: a
       tabela-âncora E120PED já fixa CODFIL=1 no WHERE, então o casamento
       dinâmico de USU_V120EST é transitivamente correto), mas o ponto
       cego em si existia -- mantido visível via ALERTA por consistência
       com as demais áreas, caso uma tabela nova entre no catálogo depois.
    b) Só comparava COUNT(*) -- nunca pegaria divergência de valor num
       registro presente dos dois lados.

NOTA: usa montar_query_contagem() (não montar_query_pks()) para a
contagem -- diferente das áreas com todas as tabelas com PK, a Expedição
tem USU_V120EST sem PK física (chaves_pk=None), e montar_query_contagem()
não depende de nenhuma coluna específica, só do filtro empresa/filial --
funciona igual para as 2 tabelas, não precisa pular nenhuma.

Só leitura -- não corrige nem apaga nada.

Uso:
    python -m conferencias.dw_bronze.conferencia_expedicao
        -> contagem (rápida) nas tabelas do catálogo.

    python -m conferencias.dw_bronze.conferencia_expedicao --conteudo USU_V120EST
...
from expedicao.bronze.extrator import TABELAS_DESTE_BLOCO, montar_query, montar_query_contagem
from expedicao.bronze.tabelas import buscar_tabela
from expedicao.config.settings import CODEMP_AQUARIO, schema_bronze
...
def contar_sapiens(engine, info: dict) -> int:
    ...
    catálogo. Funciona tanto para tabelas com PK quanto para USU_V120EST
    (sem PK) -- montar_query_contagem() não depende de chaves_pk.
    """
    query = montar_query_contagem(info)
...
if __name__ == "__main__":
    ...
    if "--conteudo" in sys.argv:
        ...
        try:
            ...
            print("  Uso: python -m conferencias.dw_bronze.conferencia_expedicao --conteudo NOME_TABELA")
            sys.exit(1)
