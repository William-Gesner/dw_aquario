...
Comercial/Laudos RMA/Produção/Estoque. Só compara as tabelas de
TABELAS_DESTE_BLOCO (extrator.py) -- as 3 compartilhadas com o Comercial
não entram aqui, já são validadas pela conferência do Comercial.

Usa montar_query_contagem() (não montar_query_pks()) -- diferente das
outras áreas, a Expedição tem uma tabela sem PK física (USU_V120EST),
e a contagem de universo completo não depende de nenhuma coluna
específica, só do filtro de empresa/filial.
...
conferência dedicada ao levar pra VM, junto com os das outras áreas):
    python -m expedicao.bronze.conferencia
"""
...
from expedicao.bronze.extrator import TABELAS_DESTE_BLOCO, montar_query_contagem
from expedicao.bronze.tabelas import buscar_tabela
from expedicao.config.settings import schema_bronze
...
def contar_sapiens(engine, info: dict) -> int:
    """
    Conta o universo completo e atual da tabela/view no Sapiens, no mesmo
