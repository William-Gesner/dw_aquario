"""
Conferência FAT_PARADAS_PRODUCAO (Prata) x USU_VBIAPROD_PARADAS (legado,
...
CORTE DE DATA: a Prata usa 01/01/2021 (DATA_CORTE_PRODUCAO), diferente
do legado (01/01/2018, DATA_INICIO_HISTORICO) -- decisão explícita do
usuário em 21/07/2026. Por isso o lado do legado também é filtrado por
DATMPR >= 01/01/2021 aqui, senão a contagem SEMPRE vai divergir por
causa do corte (proposital), mascarando uma divergência de conteúdo de
verdade. O que importa validar é se os dados de 2021 em diante batem
100% -- o legado ter mais linhas (2018-2020) é esperado, não é bug.

PRÉ-REQUISITO: GRANT SELECT ON BIAQUARIO.USU_VBIAPROD_PARADAS TO DW_PRATA;

Uso:
    python -m conferencias.dw_prata.conferencia_fat_paradas_producao
"""
...
TABELA_PRATA = "FAT_PARADAS_PRODUCAO"

SCHEMA_LEGADO = "BIAQUARIO"
TABELA_LEGADO = "USU_VBIAPROD_PARADAS"

COLUNAS = [
    "CODETG", "CODCRE", "CODCCU", "DESCCU", "IDEBEM", "DATMPR", "CODMTV",
    "DESMTV", "ABRMTV", "TIPMTV", "DESTIP", "CLAMTV", "DESCLA", "SEQMPR",
    "HORCHA", "HORINI", "HORFIM",
]

# Só a fatia do legado que também está na Prata (corte de data)
LEGADO_COM_CORTE = (
    f"(SELECT {', '.join(COLUNAS)} FROM {SCHEMA_LEGADO}.{TABELA_LEGADO} "
    f"WHERE DATMPR > TO_DATE('{DATA_CORTE_PRODUCAO}', 'DD/MM/YYYY'))"
)
