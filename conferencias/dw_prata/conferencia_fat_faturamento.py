"""
Conferência FAT_FATURAMENTO (Prata) x USU_VBIAFATURAMENTO (legado,
...
IMPORTANTE: a lista de colunas comparadas aqui é só o conjunto ORIGINAL
do legado (31 colunas) -- as 4 colunas novas e aditivas
(SEQIPD/SEQIPV/SEQISV/SEQIPC, ver comercial/prata/fat_faturamento.py)
não existem em BIAQUARIO.USU_VBIAFATURAMENTO, então entram fora da
comparação de propósito (senão a query quebraria com ORA-00904 do lado
do legado).

PRÉ-REQUISITO: GRANT SELECT ON BIAQUARIO.USU_VBIAFATURAMENTO TO DW_PRATA;

Uso:
    python -m conferencias.conferencia_fat_faturamento
"""
...
TABELA_PRATA = "FAT_FATURAMENTO"

SCHEMA_LEGADO = "BIAQUARIO"
TABELA_LEGADO = "USU_VBIAFATURAMENTO"

# Só as colunas que existem nos DOIS lados (conjunto original do legado)
COLUNAS = [
    "DATAREF", "TIPOREG", "VENDA", "CODFILIAL", "CODCLIBASE", "CODCLI",
    "NUMPED", "NUMNFV", "NUMNFD", "CHAVE_ITEM", "CODDEP", "CODTRANSP",
    "CODCPG", "CODFPG", "QTDITEM", "PRECOUNIT", "VLR_LIQ", "PERC_OFF",
    "VLR_OFF", "VLR_IPI", "VLR_ICMS", "VLR_ST", "VLR_FUNDPOB", "VLR_OUT",
    "VLR_SLI", "VLRBRUTO_TOTAL", "VLRDESC_TOTAL", "MOTIVODEV", "CODREP",
    "CODREGREP", "CODREGCLI",
]
