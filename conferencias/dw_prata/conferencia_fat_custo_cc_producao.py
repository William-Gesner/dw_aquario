"""
Conferência FAT_CUSTO_CC_PRODUCAO (Prata) x USU_VBIAPROD_CUSTOCC (legado,
...
CORTE DE DATA: a Prata usa 01/01/2021 (DATA_CORTE_PRODUCAO, sobre
DATINI) -- corte NOVO, o legado não tinha nenhum. Por isso o lado do
legado também é filtrado por DATINI >= 01/01/2021 aqui, senão a
contagem SEMPRE vai divergir por causa do corte (proposital), mascarando
uma divergência de conteúdo de verdade. O que importa validar é se os
dados de 2021 em diante batem 100% -- o legado ter mais linhas (antes de
2021) é esperado, não é bug.

PRÉ-REQUISITO: GRANT SELECT ON BIAQUARIO.USU_VBIAPROD_CUSTOCC TO DW_PRATA;

Uso:
    python -m conferencias.dw_prata.conferencia_fat_custo_cc_producao
"""
...
TABELA_PRATA = "FAT_CUSTO_CC_PRODUCAO"

SCHEMA_LEGADO = "BIAQUARIO"
TABELA_LEGADO = "USU_VBIAPROD_CUSTOCC"

COLUNAS = [
    "CODEMP", "NUMMTC", "TIPMTC", "TIPORC", "CODMTC", "DESMTC", "CODCCU",
    "CODNTG", "DESNTG", "DATINI", "DATFIM",
]

LEGADO_COM_CORTE = (
    f"(SELECT {', '.join(COLUNAS)} FROM {SCHEMA_LEGADO}.{TABELA_LEGADO} "
    f"WHERE DATINI >= TO_DATE('{DATA_CORTE_PRODUCAO}', 'DD/MM/YYYY'))"
)
