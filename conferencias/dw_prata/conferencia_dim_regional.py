"""
Conferência DIM_REGIONAL (Prata) x USU_VBIREGIONAIS (legado, schema
BIAQUARIO) -- compara os DADOS, não só a contagem.

Ver conferencia_dim_condicao_pagamento.py para a explicação completa da
técnica (MINUS nos dois sentidos). Atenção especial nesta tabela: é
onde os overrides de RESPONSAVEL (ver comercial/prata/dim_regional.py)
precisam bater exatamente com o legado -- se a conferência divergir,
comece checando as 4 regionais/login com exceção manual.

Só leitura -- não corrige nem apaga nada.

PRÉ-REQUISITO: GRANT SELECT ON BIAQUARIO.USU_VBIREGIONAIS TO DW_PRATA;

Uso:
    python -m conferencias.conferencia_dim_regional
"""
...
TABELA_PRATA = "DIM_REGIONAL"

SCHEMA_LEGADO = "BIAQUARIO"
TABELA_LEGADO = "USU_VBIREGIONAIS"

COLUNAS = [
    "ID_REGIONAL", "REGIONAL", "NOME_REGIONAL", "RESPONSAVEL",
    "ASSISTENTE", "DT_CADASTRO",
]
