"""
Motor genérico de extração da camada Bronze da Rastreabilidade.

Mesmo padrão do Comercial/Produção/Estoque: um único motor genérico
serve para a única tabela do catálogo
(rastreabilidade/bronze/tabelas.py) -- não precisa de script por tabela.
...
TABELAS COMPARTILHADAS COM O COMERCIAL (E140IPV, E140NFV, E075PRO,
E026RAM, E085CLI, E090REP, E120IPD) e COM O LAUDOS RMA (USU_VZRASLAU)
NÃO aparecem em TABELAS_DESTE_BLOCO -- já são mantidas pelo
comercial/bronze/extrator.py e laudos_rma/bronze/extrator.py
respectivamente. Ver observação em bronze/tabelas.py.

VERIFICAÇÃO DE ÓRFÃOS: mesma regra do Comercial/OPEX/Laudos RMA/
Produção/Estoque/Expedição -- só roda na ÚLTIMA execução do dia, quando
o script é chamado com a flag --sweep-orfaos.
"""
...
from rastreabilidade.bronze.tabelas import TABELAS, buscar_tabela
...
def montar_query(info: dict, primeira_carga: bool) -> str:
    ...
    nenhum filtro (não é o caso da única tabela desta área). Filtra pela
    coluna real de empresa (info["coluna_codemp"]) = 1, e pela coluna
    real de filial (info["coluna_codfil"]) = 1 quando tem_codfil = True.
