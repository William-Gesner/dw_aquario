"""
Motor genérico de extração da camada Bronze do Estoque.

Mesmo padrão do Comercial/Produção/Laudos RMA: um único motor genérico
serve para as 3 tabelas do catálogo (estoque/bronze/tabelas.py) -- não
precisa de script por tabela.
...
Para cada tabela:
    ...
    2. Monta a query de extração:
        ...
        - cargas seguintes -> idem + filtro coluna_data >= SYSDATE - 60
                              ...
                              2 das 3 tabelas desta área não têm coluna
                              de auditoria confiável, ver tabelas.py)
    3. Chama core.loader.carregar_bronze() para gravar na Bronze.

TABELAS COMPARTILHADAS COM O COMERCIAL (E075DER, E075PRO, E095FOR) e COM
A PRODUÇÃO (E210MVP) NÃO aparecem em TABELAS_DESTE_BLOCO -- já são
mantidas pelo comercial/bronze/extrator.py e producao/bronze/extrator.py
...
-- só roda na ÚLTIMA execução do dia, quando o script é chamado com a
flag --sweep-orfaos.
"""
...
from estoque.bronze.tabelas import TABELAS, buscar_tabela
from estoque.config.settings import (
    CODEMP_AQUARIO,
    CODFIL_AQUARIO,
...
def montar_query(info: dict, primeira_carga: bool) -> str:
    ...
    3 têm CODEMP). Filtra pela coluna real de empresa
